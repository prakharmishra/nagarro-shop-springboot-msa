...
public interface InventoryItemDAO extends CrudRepository<InventoryItem, Long> {
	...
	@Query("SELECT item FROM InventoryItem item JOIN item.itemInventoryAvailability avail WHERE item.id = ?1 AND avail.isActive = true AND avail.noOfItemsAvailable > 0")
	List<InventoryItem> findActiveAvailableItem(Long itemId);
	
	@Query("SELECT item FROM InventoryItem item JOIN item.itemInventoryAvailability avail WHERE avail.isActive = true")
